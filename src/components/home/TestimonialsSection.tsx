  ...
  return (
    <div className="flex gap-1 mb-6">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} width="14" height="14" viewBox="0 0 14 14" fill="currentColor" className="text-white/80">
    ...
    <section className="py-20 md:py-32 bg-[#0A0A0A]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-14 md:mb-20">
          <p className="font-sans text-[9px] tracking-[0.45em] text-[#85A0B5] uppercase mb-5">
            Client Voices
          </p>
          <h2 className="font-serif text-4xl md:text-6xl text-white">
            Worn &amp; <em className="not-italic text-[#9A9A9A]">Adored</em>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-[#1A1A1A]">
          {testimonials.map((t, i) => (
            <div
              key={i}
              className="bg-[#0F0F0F] px-7 py-8 md:px-8 md:py-10 flex flex-col"
            >
              <Stars />

              <blockquote className="font-serif text-base md:text-[17px] text-[#C4CDD6] italic leading-relaxed flex-1 mb-8">
                &ldquo;{t.quote}&rdquo;
              </blockquote>

              <footer>
                <p className="font-sans text-[10px] tracking-[0.25em] text-white uppercase mb-1">
                  {t.name}
                </p>
                <p className="font-sans text-[8px] tracking-[0.25em] text-[#3A3A3A] uppercase">
