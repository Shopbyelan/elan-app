...
async function getAccountData(userId: string) {
  const [user, orderCount, wishlistCount, addressCount] = await Promise.all([
    prisma.user.findUnique({ where: { id: userId } }),
    prisma.order.count({ where: { userId } }),
    prisma.wishlist.count({ where: { userId } }),
    prisma.address.count({ where: { userId } }),
  ]);
  return { user, orderCount, wishlistCount, addressCount };
}
...
export default async function AccountPage() {
  ...
  try {
    ...
    data = { user: null, orderCount: 0, wishlistCount: 0, addressCount: 0 };
  }

  const { user, orderCount, wishlistCount, addressCount } = data;
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-12 md:py-20">
      ...
      <div className="grid grid-cols-3 gap-px bg-[#1A1A1A] mb-6">
        {stats.map(({ label, value, icon: Icon, href }) => (
          ...
          >
            <Icon className="h-4 w-4 text-[#2A2A2A] group-hover:text-[#C9A84C] mx-auto mb-2 transition-colors" />
      ...
      <div className="bg-[#111] border border-[#1A1A1A] divide-y divide-[#141414] mb-6">
        {links.map(({ label, desc, href, icon: Icon }) => (
          ...
          >
            <Icon className="h-4 w-4 text-[#3A3A3A] group-hover:text-[#C9A84C] transition-colors flex-shrink-0" />
            ...
            <ChevronRight className="h-3 w-3 text-[#2A2A2A] group-hover:text-[#C9A84C] transition-colors flex-shrink-0" />
          </Link>
        ))}
      </div>
